...
public class ChainSmithingMenu extends ItemCombinerMenu {
    ...
    protected ItemCombinerMenuSlotDefinition createInputSlotDefinitions() {
        return ItemCombinerMenuSlotDefinition.create()
            // 4个模板槽位 (0-3)
            .withSlot(0, 8, 20, itemStack -> this.recipes.stream()
                .anyMatch(smithingRecipe -> smithingRecipe.value().isTemplateIngredient(itemStack)))
            .withSlot(1, 26, 20, itemStack -> this.recipes.stream()
                .anyMatch(smithingRecipe -> smithingRecipe.value().isTemplateIngredient(itemStack)))
            .withSlot(2, 8, 38, itemStack -> this.recipes.stream()
                .anyMatch(smithingRecipe -> smithingRecipe.value().isTemplateIngredient(itemStack)))
            .withSlot(3, 26, 38, itemStack -> this.recipes.stream()
                .anyMatch(smithingRecipe -> smithingRecipe.value().isTemplateIngredient(itemStack)))
            // 1个基础物品槽位 (4)
            .withSlot(4, 62, 29, itemStack -> this.recipes.stream()
                .anyMatch(smithingRecipe -> smithingRecipe.value().isBaseIngredient(itemStack)))
            // 4个材料槽位 (5-8)
            .withSlot(5, 98, 20, itemStack -> this.recipes.stream()
                .anyMatch(smithingRecipe -> smithingRecipe.value().isAdditionIngredient(itemStack)))
            .withSlot(6, 116, 20, itemStack -> this.recipes.stream()
                .anyMatch(smithingRecipe -> smithingRecipe.value().isAdditionIngredient(itemStack)))
            .withSlot(7, 98, 38, itemStack -> this.recipes.stream()
                .anyMatch(smithingRecipe -> smithingRecipe.value().isAdditionIngredient(itemStack)))
            .withSlot(8, 116, 38, itemStack -> this.recipes.stream()
                .anyMatch(smithingRecipe -> smithingRecipe.value().isAdditionIngredient(itemStack)))
            // 1个结果槽位 (9)
            .withResultSlot(9, 152, 29)
            .build();
    }
